...
const Store = ({ user }) => {
  ...
  return (
    <Container className="py-5">
      <h1 className="mb-5 text-center">Toko Kami</h1>
      <p className="lead text-center mb-4">Lihat koleksi rajutan kami:</p>
          ...
          rajutan.map((item) => (
            <Col key={item.id}>
              <Card className="h-100 shadow-sm">
                {item.url_gambar && (
                  <Card.Img variant="top" src={item.url_gambar} className="card-img-top-custom" />
                )}
                <Card.Body className="text-center d-flex flex-column">
                  <Card.Title className="fw-bold fs-5 mb-2">{item.nama}</Card.Title>
                  ...
                  <Card.Text className="text-secondary small">Warna: {item.colorcode.join(', ')}</Card.Text>
                  <Button
                    disabled={isGuest}
                    onClick={() =>
                      isGuest
